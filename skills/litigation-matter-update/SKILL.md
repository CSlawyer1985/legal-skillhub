...
1. 按以下工作流操作。
2. 确认代号在 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/` 和 `_log.yaml` 中存在。
3. 提示输入：事件类型、日期（默认今天）、摘要，以及任何日志字段更新（风险变更、状态变更、下一节点变更、重要性重新分类）。
4. 追加带日期条目至 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/history.md`。
...
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/matter.md` —— 参考（不重写）
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` —— 风险校准（如需重评估风险）

**冲突门禁——不可绕过。** 在记录更新前，检查 `_log.yaml` 中是否存在该案件代号。如果案件不在 `_log.yaml` 中，拒绝并路由：

> "我在案件日志中没有找到 [案件代号]。请先运行 `litigation-matter-intake`，以便冲突检索可以运行且案件工作空间已经建立。我不会向未登记的案件追加历史记录——冲突检索是门禁，且在案件完成登记之前没有 `history.md` 可供追加。"
...
如果策略更新为**接受和解**（公司正在接受和解要约、签署和解协议或原则性授权接受——不仅仅是记录要约的发出或收到）：读取 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 中的 `## 使用者`。如果角色是**非律师**：

> 接受和解具有法律后果——它解决争议，通常需要签署免责协议，并可能影响保险、税务和关联事项。您是否已与律师审查过此事？如已审查，继续。如未审查，以下是带去给律师的简要材料：
>
> [生成一页摘要：案件、拟议和解条款（金额、结构、免责范围、保密、不得贬损条款）、涉及的敞口、授权层级状态（见 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 和解授权）、可能出错的事项、接受前需要问律师的问题。]
>
...
如重要性变更为"需计提"或"已披露"，且该案件此前未记录计提或披露，标注该事件需要按 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 的重要性门槛通知财务/审计委员会。
...
### 追加至 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/history.md`
