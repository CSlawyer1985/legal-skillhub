---
name: corporate-entity-compliance
...
1. 加载 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` → `## 主体管理`（主体清单、注册地、工商登记代办机构）。
2. 按以下标志路由到正确的模式：
...
3. 读取/写入 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/entities/compliance-tracker.yaml`。
...
工商年报、企业所得税年度申报、信息公示、两年度申报——每个主体在每个注册地有自己的时间安排和错过截止日的各自后果。本技能维护一个单一的 YAML 追踪器，知道什么到期、何时到期、针对哪个主体。有意设计为轻量级：追踪器是你拥有的文件，本技能按指令更新，需要分享时你导出。
...
> 本追踪器按每个主体记录的设立地或经营地计算截止日。申报规则、截止日机制和费用结构在不同注册地之间存在实质性差异。如果主体的实际业务范围与 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` 中记录的不同（未披露的外地经营备案、已注销主体、注册地迁移、由本地代理机构管理的跨省级申报），输出可能不完全适用——与工商登记代办机构或该辖区的当地律师确认。
...
> **中国法下关键区别：**
>
> - **有限责任公司/股份有限公司：** 须于每年1月1日至6月30日通过国家企业信用信息公示系统报送并公示年度报告。法律依据：国务院《企业信息公示暂行条例》第7、8条 `[模型知识 — 需核验]`。
> - **外商投资企业：** 除年报外，还需根据《外商投资法》及其实施细则完成外商投资信息报告。法律依据：《外商投资法》第34条 `[模型知识 — 需核验]`。
...
存放于 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/entities/compliance-tracker.yaml`。结构：
...
读取 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` → `## 主体管理` → 主体清单。如果主体清单已填充（来自冷启动时的组织架构图上传），直接使用。如果未填充，请用户运行冷启动模块或提供主体清单。
...
生成 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/entities/compliance-tracker.yaml`，包含全部主体及其计算出的申报要求。设置初始状态：
...
运行 corporate-entity-compliance --report 查看待办事项。
```
...
```
corporate-entity-compliance --report [--days 30|60|90|180]
```
...
**在指示或确认一项申报前：** 读取 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` 中的 `## 使用者`。如果角色为**非法务人员**：
...
>
> 如需寻找律师：联系中华全国律师协会或所在地地方律师协会获取推荐服务 `[模型知识 — 需核验]`。
...
```
corporate-entity-compliance --update
```
...
```
corporate-entity-compliance --update --from-report
```

用户上传工商登记代办机构或类似合规报告（PDF、CSV 或 Excel）。本技能读取并更新匹配的主体：
...
```
corporate-entity-compliance --sweep
```
...
```
corporate-entity-compliance --audit
```
...
**经营备案缺口：**
- 基于 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` 的主体清单：公司的业务足迹中是否有省份（办事处、员工）主体未办理经营备案？这需要律师确认业务存在——本技能可以提出问题但不能独立判断业务存在。

**关联方交易协议订立情况：**
- 从 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md`：如果关联方交易协议订立情况标记为部分或否，标记哪些主体关系可能需要协议（母子公司服务、知识产权许可、贷款）。
...
潜在缺口
  经营备案：[标记问题——确认以下地区的业务存在：]
    [足迹在追踪器中未显示为已备案的省份列表，来自 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md`]
  关联方交易协议订立：[来自 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` 的状态]
...
```
corporate-entity-compliance --export [--format csv|table]
```
