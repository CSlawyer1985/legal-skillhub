...
API Key 格式：`ps_` + 32 位十六进制字符串。不要在文档、报告或日志中保存真实 Key。

```
Authorization: Bearer ps_<API_KEY>
```

测试用Key：ps_0931e2efa48df3aa2596de57c27d9449
⚠️ **示例Key可能已过期或积分耗尽，请使用自己申请的 Key。**
...
4. 将API Key告诉Workbuddy或者其他本地Agent，并最好让它记住

### 脚本中如何使用 API Key
在调用时通过 `-H "Authorization: Bearer ps_你的Key"` 或环境变量 `PATSEEK_API_KEY` 传入
...
下表是当前服务端默认配置。账户优惠、免费策略或运行时配置可能改变实际扣费；客户端和报告必须以响应中的 `credits_charged` 为准。
...
| `market` | string | 否 | 专利库：`cn`（默认）或 `world`。两库接受相同表面语法，但底层字段映射/分析器可能不同，应分别抽样验证。`world` 库的 `AN=` 字段暂无数据，查询返回0条 |
...
**字段前缀：**
- `AP=(...)` — 申请人（match_phrase）
- `IPC=(...)` — IPC 分类号；准确下位组用于查准，前4位上位组用于补召回，均需抽样验证
- `PID=(...)` — 公开号（term 精确）
- `AN=(...)` — 申请号（13 位去末位校验位；`market=world` 库暂无此字段数据）
- `AD` / `PD` — 申请日/公开日（range，支持 >=、>、<、<=、=、范围）
- `NOT=(...)` — 排除（must_not）
- `CC=(...)` — 国家/地区代码（仅 `market=world` 支持，如 `CC=(US)`）
...
- 排除: `固态电池 NOT=(液态)`
- 国家限定（需 `market: "world"`）: `battery CC=(US)`
...
| `include_enrichment` | bool | 否 | 默认 `true`；补充发明人、优先权、法律状态、同族与引证摘要 |

核心字段始终以 Bool/OpenSearch 主源为准；增强源只补充 `enrichment`，超时、未命中或身份不一致时仍返回主源详情。`aggregation.status` 可能为：

- `complete`：双源聚合成功
- `primary_only`：增强源暂不可用，已返回主源详情
- `identity_mismatch`：双源公开号不一致，增强数据已安全丢弃
- `disabled`：请求或运行时配置关闭了增强

增强摘要字段：`inventors`, `priority_date`, `prior_art_date`, `legal_status`, `latest_legal_event`, `legal_event_count`, `family_count`, `citation_count`, `cited_by_count`, `similar_count`, `keywords`, `pdf_url`, `canonical_url`。
...
### 详情增强字段
`/v1/patent/{identifier}` 的 `patent_list[0]` 还可能包含：

- `enrichment`：发明人、优先权、法律状态、关系数量、PDF 与来源页
- `aggregation`：聚合状态、数据源、是否命中增强缓存

增强字段均为可选；客户端必须兼容字段不存在或 `aggregation.status=primary_only`，且不应因此重复调用详情接口。
...
| 422 | INVALID_PATENT_IDENTIFIER | 专利号/申请号为空、过长或包含不支持字符 |
