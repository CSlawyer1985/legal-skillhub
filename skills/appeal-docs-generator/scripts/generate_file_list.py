...
用法：
  python3 generate_file_list.py <模板.md> <输出.docx> [--case "案件：XX诉XX XX纠纷案"] [--counterparties N]
  --counterparties N：对方当事人人数（默认 1）。对模板中备注列标「随对方当事人数」的行，份数自动 = N + 2（我方1+对方N+法院1）；默认 3 份，多一名对方当事人多一份。
"""
...
MARK_COUNTERPARTIES = "随对方当事人数"  # 备注列标记：该行份数随对方当事人人数浮动（仅内部识别，输出时剥离，不外显）


def parse_table(md_path, counterparties=1, skip=None):
    """解析 markdown 表格为二维数组；备注含「随对方当事人数」的行，份数按 对方当事人人数+2 调整，
    并将「随对方当事人数」标记从备注中剥离（内部规则不写入交付文件）。skip 为需剔除的序号列表。"""
    rows = []
    ...
    for line in lines:
        ...
        if s.startswith("|"):
            ...
            if skip and len(cells) >= 1 and cells[0] in skip:
                continue
            if len(cells) >= 5 and MARK_COUNTERPARTIES in cells[4]:
                cells[3] = str(counterparties + 2)  # 我方1 + 对方N + 法院1
                cells[4] = re.sub(r"[；;]?\s*" + MARK_COUNTERPARTIES + r"\s*[；;]?", "", cells[4]).strip(" ；;")
            rows.append(cells)
        elif in_table:
            break
    return rows
...
def render(md_path, out_path, case_line="", counterparties=1, skip=None, add_rows=None):
    # 表格
    rows = parse_table(md_path, counterparties=counterparties, skip=skip)
    if add_rows:
...
def self_check(doc_path, expected_rows):
    ...
    if MARK_COUNTERPARTIES in joined:
        return False, "内部标记「随对方当事人数」泄露到交付文件"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--counterparties", type=int, default=1, help="对方当事人人数（默认 1），标「随对方当事人数」的行份数 = 对方当事人人数 + 2")
    ...
    render(args.template, args.output, args.case, args.counterparties, skip=skip, add_rows=args.add)
