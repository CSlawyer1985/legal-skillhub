...
监管交易卡的完整实现模式。根据具体法规和受众调整章节名称、内容和配色方案。
...
实体内容位于附件时包含附件引用：`Art. 6(2); Annex III No. 5`。
...
不要将本模式用于机器翻译的法律文本。两个语言版本必须各自独立准确。

## 可访问性

- 全程使用语义化 HTML5（`<header>`、`<nav>`、`<section>`、带正确 `<thead>`/`<tbody>` 的 `<table>`）
- 导航项和交互控件上使用 `aria-label`
- 键盘导航：所有交互元素必须可通过 Tab 键到达，并通过 Enter 或 Space 键激活
- 焦点指示器可见（不要使用没有可见替代方案的 `outline: none`）
- 颜色不是严重程度的唯一信号：风险徽章包含文字，而不仅仅是颜色
- 对比度比率至少达到 WCAG AA

## 性能

- 将所有内容内联在单个文件中。外部依赖违背了可移植交易卡的初衷。
- 尽量减少 JavaScript 复杂度。此处的交互很轻量；不需要框架。
- 用真实内容量测试（法规每个面板可产生 60 多行）。
- 确保移动设备上的初始渲染快速。
