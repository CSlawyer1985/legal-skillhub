---
name: litigation-matter-briefing
...
1. 加载 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` → 风险校准 + 相关方。
2. 按以下工作流操作。
3. 读取 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/matter.md` + `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/history.md` + `_log.yaml` 中的日志行。
...
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/_log.yaml` —— 结构化行
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/matter.md` —— 记述式登记
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/history.md` —— 事件日志
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` —— 风险校准（使"风险：高"有具体含义，而非泛泛）

**冲突门禁——不可绕过。** 在生成简报前，检查 `_log.yaml` 中是否存在该案件代号。如果案件不在 `_log.yaml` 中，拒绝并路由：

> "我在案件日志中没有找到 [案件代号]。请先运行 `litigation-matter-intake`，以便冲突检索可以运行且案件工作空间建立。我不会为未登记的案件生成简报——冲突检索是门禁。"
...
如 `last_updated > 30天前`：在顶部标注并建议会议后运行 `litigation-matter-update [slug]` 以记录讨论内容。
