...
1. 优先使用元力平台已经绑定的 `yuandian-company`，普通 Skill 会话不得修改全局 MCP 配置或重启网关。
2. 调用 `tools/list`（能力发现），确认 26 项企业能力及当前名称。
3. 读取运行时工具模式（runtime tool schema）；工具名、参数类型、枚举和必填项以运行时工具模式为准，即 source of truth。
4. 使用企业候选检索的最小返回量探测鉴权，不用聚合或大列表接口做健康检查。
5. 只有探测成功才进入企业查询。
