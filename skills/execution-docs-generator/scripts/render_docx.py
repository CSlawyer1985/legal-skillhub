...
def _add_para(doc, text, style_dict, indent=True):
    ...
    # font — support [b]...[/b] inline bold tags
    font_name = style_dict.get("font", "仿宋")
    font_size = style_dict.get("size")
    global_bold = style_dict.get("bold", False)
    
    # Split text by [b]...[/b] boundaries
    segments = re.split(r'\[b\](.*?)\[/b\]', text)
    for i, seg in enumerate(segments):
        ...
        # Odd indices (i=1,3,5...) are [b] content — always bold
        # Even indices (i=0,2,4...) are normal text — bold only if global_bold
        if i % 2 == 1:
            run.bold = True
        elif global_bold:
            run.bold = True
