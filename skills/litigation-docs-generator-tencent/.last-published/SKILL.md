...
2. **文件清单排第一**，用 `scripts/generate_file_list.py` 生成 Word 版：
...
| 07 | `07 律师工作反馈卡.docx` |
| 08 | `08 接待笔录.docx` |
| 09 | `09 营业执照（副本）.docx`（复印件，用户自备） |
| 10 | `10 法定代表人身份证（复印件）.docx`（用户自备） |

   - 清单未生成的文件（证据清单、风险告知书、律师工作反馈卡、营业执照、身份证复印件等）按清单要求由用户自备，不强制生成
   - 委托代理合同不在此编号体系内，命名 `08-委托代理合同.doc`（委托签署材料，不计入立案清单）
