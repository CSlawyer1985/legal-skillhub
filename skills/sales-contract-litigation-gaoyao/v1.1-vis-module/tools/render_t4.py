...
def box_list(s, items, x, y0, boxw, size, fillc, strokec, textc, aid_c, maxw):
    ...
    for it in items:
        ...
        s += ts
        s += f'<text x="{x+boxw-10}" y="{y+bh-8}" text-anchor="end" font-family="{FONT}" font-size="11" fill="{aid_c}">{esc("/".join(it["anchor_ids"]))}</text>'
        y += bh + 12
    return s, y

def render_01(cv, title):
    v = cv["views"]["01-主体关系图"]
    s = svg_head(title, "数据源：第一节 主体与角色（A01）＋第二节 合同与交易链（A02）")
...
def render_02(cv, title):
    ...
    s = svg_head(title, "数据源：第二至五节含日期事实行（A02-A05）；无日期节以数据行摘要入图")
    evs = v["events"]
    ...
    if evs:
        ...
        for i, e in enumerate(evs):
            ...
            s += ts
            ts, _ = tspans(lx, ny+2, ["/".join(e["anchor_ids"])], 10, "#8aa", anchor="middle")
            s += ts
    my = 620
    if ms:
        s += f'<text x="90" y="{my-28}" font-family="{FONT}" font-size="16" font-weight="bold" fill="#555">未系日期事实（数据行全文）</text>'
        for m in ms:
            lines = wrap("◇（" + "/".join(m["anchor_ids"]) + "）" + m["text"], 92)
...
def render_03(cv, title):
    ...
    s = svg_head(title, "数据源：第六节 要件-事实-证据矩阵（A06）＋第四节 款项（A04）＋第七节 证据索引（A07）")
    y = 126
    s += f'<text x="60" y="{y}" font-family="{FONT}" font-size="17" font-weight="bold" fill="#333">要件矩阵（A06）</text>'
    y += 12
    ...
    for r in v["matrix"]:
        ...
        for j in range(5):
            ...
            s += ts
            x += colw[j]
        s += f'<text x="{x+6}" y="{y+16}" font-family="{FONT}" font-size="11" fill="#8aa">A06</text>'
        y += rh
    y += 26
    s += f'<text x="60" y="{y}" font-family="{FONT}" font-size="17" font-weight="bold" fill="#333">款项与余额（A04）</text>'
    y += 12
    aw = [180, 240, 520, 470]
    ...
    for r in v["amounts"]:
        ...
        for j in range(4):
            ...
            s += ts
            x += aw[j]
        s += f'<text x="{x+6}" y="{y+16}" font-family="{FONT}" font-size="11" fill="#8aa">A04</text>'
        y += rh
    y += 26
    s += f'<text x="60" y="{y}" font-family="{FONT}" font-size="17" font-weight="bold" fill="#333">证据索引（A07）</text>'
    y += 20
    for e in v["evidence_index"]:
        lines = wrap("· " + e["text"], 100)
        ts, y = tspans(60, y, lines, 13, "#444", lh=20)
        s += ts
        s += f'<text x="1500" y="{y-20}" font-family="{FONT}" font-size="11" fill="#8aa">A07</text>'
        y += 6
    return s + "</svg>"

def render_04(cv, title):
    v = cv["views"]["04-阶段计划与风险"]
    s = svg_head(title, "数据源：第八节 诉讼阶段计划（A08）＋第九节 风险与暂停事项（A09）")
    s += f'<text x="60" y="122" font-family="{FONT}" font-size="18" font-weight="bold" fill="#333">阶段计划（A08）</text>'
    ...
    s += f'<text x="810" y="122" font-family="{FONT}" font-size="18" font-weight="bold" fill="#7a2b1a">风险与暂停事项（A09）——待核/暂停口径原样保留</text>'
