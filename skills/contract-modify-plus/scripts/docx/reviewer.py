...
class ContractReviewer:
    ...
    def __init__(self, unpacked_dir, author="WPS", initials="WP"):
        """
        初始化合同审查器

        Args:
            unpacked_dir: 解包后的 DOCX 目录路径
            author: 批注/修订的作者名称（默认由 config/reviewer_profile.json 注入，当前为 "WPS"）
            initials: 作者缩写（默认由 config/reviewer_profile.json 注入，当前为 "WP"）
    ...
    def replace_text_via_paragraph(
        ...
        """兼容别名: apply_modify_plan.py 以 current_text/proposed_text 调用, 内部转发至 _rewrite 实现。"""
        return self.replace_text_via_paragraph_rewrite(
            old_text, new_text, occurrence=None, comment_text=comment_text
        )
