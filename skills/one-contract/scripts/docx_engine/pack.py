...
def validate_document(doc_path):
    """Package validation: required files, CRC, duplicate names, and all XML parts."""
    try:
        with zipfile.ZipFile(doc_path, "r") as archive:
            all_names = archive.namelist()
            names = set(all_names)
            if len(all_names) != len(names):
                print("Validation error: duplicate ZIP members", file=sys.stderr)
                return False
            corrupt = archive.testzip()
            if corrupt:
                print(f"Validation error: CRC failed for {corrupt}", file=sys.stderr)
                return False
