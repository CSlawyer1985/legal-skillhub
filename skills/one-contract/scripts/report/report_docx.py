...
PRIMARY_COLOR = "000000"   # 黑：标题与强调（公文黑白）
ACCENT_COLOR = "404040"    # 深灰：次强调（近黑的灰）
ALT_FILL = "FFFFFF"        # 白：表头/底纹填充
BORDER_COLOR = "BFBFBF"    # 浅灰：边框与分割线

BODY_FONT_EAST_ASIA = "仿宋_GB2312"
BODY_FONT_ASCII = "Times New Roman"
HEADING_FONT_EAST_ASIA = "仿宋_GB2312"
...
def _field_run_xml(instruction: str, fallback_text: str, *, size: int = 24) -> str:
...
def _paragraph_xml(
    ...
    *,
    ...
    if kind == "title":
        ...
        shd_xml = f'<w:shd w:val="clear" w:color="auto" w:fill="FFFFFF"/>'
...
def _finding_table_xml(title: str, rows: list[str]) -> str:
    ...
    table_rows = [
        "<w:tr>"
        f"{_table_cell_xml(title, width=total_width, fill='FFFFFF', grid_span=2, role='header')}"
        "</w:tr>"
    ]
