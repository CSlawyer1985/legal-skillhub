...
class ReviewTimeline:
    ...
    def _resolve_start_time(self) -> datetime:
        ...
        # Revision metadata is an execution timestamp, not a simulated human
        # editing history.  Never mint future times to make a run look longer.
        resolved = get_local_now()
        return self._normalize_timestamp_floor(resolved)
    ...
    def start_finding(self):
        finding_start = self._resolve_start_time()
        self._active_finding_start = finding_start

        def provider():
            if self.start_at is not None:
                return finding_start
            return self._resolve_start_time()
