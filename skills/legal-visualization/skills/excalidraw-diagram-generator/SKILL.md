...
> **子技能 / 随主包分发：** 本目录位于 `skills/excalidraw-diagram-generator/`，与主技能 **legal-visualization（法律图表化）** 同包安装；由主技能按需加载本文件。主技能 ZIP 已包含本目录，无需再单独安装 Excalidraw 技能包。若你手动只拷贝本目录到全局 `skills/excalidraw-diagram-generator/`，亦可作为独立技能使用。
...
### Legal Diagram Style Override

When creating legal diagrams, litigation maps, case relationship diagrams, judgment diagrams, contract-chain diagrams, or diagrams for lawyers:

1. Use clean whiteboard style, not messy sketch style.
2. Use `fillStyle: "solid"` for rectangles and important nodes.
3. Avoid `hachure` and `cross-hatch` fills unless the user explicitly asks for rough sketch shading.
4. Keep `roughness` at `0` or `1`.
5. Use colored strokes/arrows and light background colors to express legal meaning.
6. Avoid large curved dashed loops; use lanes, hubs, or split into multiple diagrams.
7. Ensure arrows do not cross text or pass through unrelated boxes.
8. Keep node text short: role, name, amount/result.
