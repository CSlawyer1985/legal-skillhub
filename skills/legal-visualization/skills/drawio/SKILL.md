...
> **子技能 / 随主包分发：** 本目录位于 `skills/drawio/`，与主技能 **legal-visualization（法律图表化）** 同包安装；由主技能按需加载本文件。主技能 ZIP 已包含本目录，无需再单独安装 draw.io 技能包。若你手动只拷贝本目录到全局 `skills/drawio/`，亦可作为独立技能使用。
