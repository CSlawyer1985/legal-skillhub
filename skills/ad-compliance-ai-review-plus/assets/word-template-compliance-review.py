...
def _add_cover_page(doc, title, cover_meta):
    ...
    # 当 cover_meta 为 None 时，不添加任何默认日期
...
def _set_formatted_text(paragraph, text):
    ...
    支持 **粗体** 内嵌在普通文本中的任意位置。
    修复：确保Emoji字符（如🔴🟡🟢）能正确显示，使用支持Emoji的字体。
    """
    import re

    # 辅助函数：检测字符是否为Emoji（含彩色圆点）
    def is_emoji_char(c):
        """检测单个字符是否为Emoji"""
        if not c:
            return False
        ...
        emoji_ranges = [
            ...
            (0x1F000, 0x1F02F), # 麻将牌
            (0x1F0A0, 0x1F0FF), # 扑克牌
            (0x1F100, 0x1F64F), # 带圈字符
            (0x1F680, 0x1F6FF), # 交通工具
            (0x1F910, 0x1F96B), # 补充表情符号
            (0x1F980, 0x1F9E0), # 补充表情符号第二部分
        ]
    ...
    # 辅助函数：为run设置字体（根据是否包含Emoji）
    def apply_font_to_run(run, text_segment):
        """为run设置合适的字体"""
        has_emoji = any(is_emoji_char(c) for c in text_segment)

        if has_emoji:
            # 包含Emoji的段落：使用支持Emoji的字体
            # Windows: 'Segoe UI Emoji', macOS: 默认通常支持
            try:
                run.font.name = 'Segoe UI Emoji'
                run._element.rPr.rFonts.set(qn('w:eastAsia'), 'Segoe UI Emoji')
            except:
                # 如果设置失败，使用默认字体
                run.font.name = 'SimSun'
                run._element.rPr.rFonts.set(qn('w:eastAsia'), 'SimSun')
    ...
    for part in parts:
        ...
        if part.startswith('**') and part.endswith('**'):
            ...
            run.font.bold = True
            apply_font_to_run(run, inner)
        else:
            # 将普通文本按Emoji/非Emoji拆分为多个run
            i = 0
            while i < len(part):
                if is_emoji_char(part[i]):
                    # Emoji字符单独一个run
                    run = paragraph.add_run(part[i])
                    apply_font_to_run(run, part[i])
                    i += 1
                else:
                    # 收集连续的非Emoji字符
                    j = i
                    while j < len(part) and not is_emoji_char(part[j]):
                        j += 1
                    segment = part[i:j]
                    run = paragraph.add_run(segment)
                    apply_font_to_run(run, segment)
                    i = j
