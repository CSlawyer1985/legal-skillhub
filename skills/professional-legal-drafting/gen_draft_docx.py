#!/usr/bin/env python3
"""
将 accurLex 知法返回的文书生成结果转为 Word 文档。

核心原则：**文书生成原文不做任何内容修改，只做展示形式处理**。

用法:
  python3 gen_draft_docx.py <input.json> [--output <output.docx>]
  python3 gen_draft_docx.py <input.md>   [--output <output.docx>]

input.json: task_draft_generate.js 输出的结构化 JSON
input.md:   task_draft_generate.js 输出的 Markdown 原文
...
def generate_draft_docx(input_path, output_path=None):
    """
    从 JSON 或 Markdown 文件生成法律文书 Word 文档。
    **文书内容原文不修改，只做展示形式处理。**
    """
    # 确定输出路径
    if not output_path:
    ...
    if ext == '.json':
        ...
        if error:
            print(f'[error] 文书生成失败: {error} {error_message}', file=sys.stderr)
            sys.exit(1)
        
        # 组装完整 Markdown（与 JS 脚本 formatRawOutput 一致）
        full_markdown = draft_text
        if citations:
    ...
    # 文档标题
    add_heading_styled(doc, '法律文书', level=0)
    ...
    print(f'[ok] 法律文书 Word 已生成: {output_path}', file=sys.stderr)
    return output_path


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='将 accurLex 文书生成结果转为 Word 文档（原文不改内容）')
    parser.add_argument('input', help='输入文件路径（.json 或 .md）')
    parser.add_argument('--output', '-o', help='输出 .docx 文件路径（默认同目录同名）')
    args = parser.parse_args()
    
    generate_draft_docx(args.input, args.output)