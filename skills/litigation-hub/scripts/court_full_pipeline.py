...
# 身份证号（18位，第7-14位为出生日期）。用于在搜电话前屏蔽，避免身份证被误当电话
ID_CARD_RE = re.compile(r'\d{6}(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{3}[\dXx]')


def _strip_id_cards(text):
    """屏蔽文本中的身份证号，防止被当作电话号码提取。"""
    return ID_CARD_RE.sub(' ', text)
...
def parse_sms(text):
    ...
    # 有送达平台链接即为文书送达（正文措辞可能用"签收"等，不依赖关键词）
    if info['download_url'] and info['sms_type'] == 'info_notification':
        info['sms_type'] = 'document_delivery'
...
def parse_pdfs(files):
    ...
    for f in files:
        ...
        if text:
            ...
            # 开庭时间（兼容「应到时间 2026 年 09 月 14 日 09:00」空格分隔、无冒号写法）
            m = re.search(r'(?:应到时间|开庭时间)\s*[：:]?\s*(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日.{0,10}?(\d{1,2})[：:](\d{2})', text)
            if m:
                g = m.groups()
                doc_info['hearing_time'] = f"{g[0]}-{g[1].zfill(2)}-{g[2].zfill(2)} {g[3].zfill(2)}:{g[4].zfill(2)}"

            # 地点（兼容「应到处所 第九审判庭」空格分隔、无冒号写法）
            m = re.search(r'(?:应到处所|开庭地点)\s*[：:]?\s*(.+?)(?:\n|$)', text)
            if m:
            ...
            if not m:
                # 无"案由："标签时，从"XX纠纷/争议一案"句式提取
                m = re.search(r'([\u4e00-\u9fff]{2,20}?(?:纠纷|争议))(?:一案)', text)
            if m:
                doc_info['case_type'] = m.group(1).strip().rstrip('一案')

            # 法官
            m = re.search(r'(?:审判[员长]|审判员|审判长|承办法官|执行法官|执行员)\s*[：:]?\s*([\u4e00-\u9fff]{2,4})', text)
            if m:
                name = m.group(1).strip()
                # 去掉"独任审理"误捕的"独"字（如"审判员某法官独任审理"→"某法官"）
                if name.endswith('独') and text[m.end(1):m.end(1) + 1] == '任':
                    name = name[:-1]
                doc_info['judge_name'] = name

            # 书记员
            m = re.search(r'书记员\s*[：:]\s*([\u4e00-\u9fff]{2,4})', text)
            if m:
                doc_info['clerk_name'] = m.group(1).strip()

            # 联系人（传票中常见"联系人：姓名+电话"）
            m = re.search(r'联系人\s*[：:]\s*([\u4e00-\u9fff]{2,4})\s*(\d[\d\-()（）\s]{5,19})', text)
            if m:
                doc_info['contact_name'] = m.group(1).strip()
                doc_info['contact_phone'] = re.sub(r'[\s\-()（）]', '', m.group(2).strip())

            # 法院人员联系信息（传票/受理通知书特有关键字段）
            # 从 court_contacts 模块提取
            if doc_info['document_type'] in ('传票', '受理案件通知书', '应诉通知书', '裁定书', '举证通知书'):
                try:
                    from court_contacts import extract_from_pdf_text
                    contacts = extract_from_pdf_text(text, filename=f['name'])
                    doc_info['court_contacts'] = contacts
                except ImportError:
                    pass

        # 兜底：文字层提取失败（扫描件）→ 用更粗的规则从已知元数据拼联系人
        if not doc_info.get('court_contacts') and not doc_info.get('has_text'):
            contacts_fallback = []
            names = []
            if doc_info.get('judge_name'):
                names.append((doc_info['judge_name'], '审判员'))
            if doc_info.get('clerk_name'):
                names.append((doc_info['clerk_name'], '书记员'))
            if names:
                # 从全部文字中暴力搜索电话号码（先屏蔽身份证号，避免误当电话）
                phones = re.findall(r'(?:0\d{2,3}[-]?\d{7,8}|\d{11})', _strip_id_cards(text))
                phones = [re.sub(r'[\s\-()（）]', '', p) for p in phones if len(re.sub(r'[\s\-()（）]', '', p)) >= 7]
                if phones and names:
                    for (name, role) in names:
                        contacts_fallback.append({
                            'name': name, 'phone': phones[0], 'role': role
                        })
                    doc_info['court_contacts'] = contacts_fallback
...
def archive_to_desktop(sms_info, docs, files):
    ...
    with open(text_path, 'w', encoding='utf-8') as f:
        ...
        for d in docs:
            ...
            if d.get('judge_name'):
                f.write(f"  法官/审判员: {d['judge_name']}\n")
            if d.get('clerk_name'):
                f.write(f"  书记员: {d['clerk_name']}\n")
            if d.get('court_contacts'):
                for c in d['court_contacts']:
                    f.write(f"  联系人: {c.get('role', '')} {c.get('name', '')} - {c.get('phone', '')}\n")
...
def save_court_contacts(docs, court_name='', case_no='', case_desc=''):
    """
    将文书解析出的法院人员（法官/书记员）保存到 macOS 通讯录。
    仅在识别到传票或受理案件通知书时调用。
    ...
    for d in docs:
        if d.get('court_contacts'):
            for c in d['court_contacts']:
                c['court'] = c.get('court', '') or court_name
                c['case_no'] = c.get('case_no', '') or case_no
                c['case_desc'] = c.get('case_desc', '') or case_desc
                all_contacts.append(c)

    # 兜底：文字层提取失败时，用解析出的元数据拼联系人
    if not all_contacts:
        for d in docs:
            names = []
            if d.get('judge_name'):
                names.append((d['judge_name'], '审判员', d.get('judge_phone', '')))
            if d.get('clerk_name'):
                names.append((d['clerk_name'], '书记员', d.get('clerk_phone', '')))
            if d.get('contact_name'):
                names.append((d['contact_name'], '联系人', d.get('contact_phone', '')))

            # 尝试从全文暴力搜索电话（先屏蔽身份证号，避免误当电话）
            text = _strip_id_cards(d.get('_raw_text', ''))
            phones_raw = re.findall(r'(?:0\d{2,3}[-]?\d{7,8}|\d{11})', text)
            phones = [re.sub(r'[\s\-()（）]', '', p) for p in phones_raw if len(re.sub(r'[\s\-()（）]', '', p)) >= 7]

            for (name, role, known_phone) in names:
                phone = known_phone or (phones[0] if phones else '')
                if phone and name:
                    all_contacts.append({
                        'name': name, 'phone': phone,
                        'role': role, 'court': court_name, 'case_no': case_no,
                        'case_desc': case_desc
                    })

            # 最后手段：有电话但没名字
            if not all_contacts and phones:
                all_contacts.append({
                    'name': names[0][0] if names else court_name,
                    'phone': phones[0],
                    'role': names[0][1] if names else '法院',
                    'court': court_name, 'case_no': case_no,
                    'case_desc': case_desc
                })

    if not all_contacts:
        return {'saved': 0, 'message': '未提取到联系人信息'}

    # 同号合并：同一电话号码只存一条通讯录，多人信息合并到备注
    ROLE_ORDER = {'审判员': 0, '审判长': 0, '承办法官': 1, '法官': 2, '执行法官': 2, '执行员': 3, '书记员': 4, '法官助理': 5, '联系人': 6}
    merged_by_phone = {}  # phone_digits -> {name, phone, roles[], cases[]}
    for c in all_contacts:
        phone_clean = re.sub(r'\D', '', c.get('phone', ''))
        if not phone_clean:
            continue
        if phone_clean not in merged_by_phone:
            merged_by_phone[phone_clean] = {
                'phone': phone_clean,
                'names': [],       # [(name, role)]
                'courts': set(),
                'case_descs': [],
        ...
        m = merged_by_phone[phone_clean]
        name = c.get('name', '')
        role = c.get('role', '')
        if name and role:
            m['names'].append((name, role))
            rank = ROLE_ORDER.get(role, 99)
            if rank < m['best_rank']:
                m['best_rank'] = rank
                m['best_name'] = name
                m['best_role'] = role
        if c.get('court'):
            m['courts'].add(c['court'])
        if c.get('case_desc') and c['case_desc'] not in m['case_descs']:
            m['case_descs'].append(c['case_desc'])
        if c.get('case_no') and c['case_no'] not in m['case_nos']:
            m['case_nos'].append(c['case_no'])

    # 重建合并后的联系人列表
    merged_contacts = []
    for phone_clean, m in merged_by_phone.items():
        # 联系人名：多人共享时用法院名；单人时用人名
        if len(m['names']) > 1:
            display_name = court_name_merged or m['best_name']
        else:
            display_name = m['best_name']

        # 拼接完整的备注
        note_parts = []
        for (n, r) in m['names']:
            note_parts.append(f"{r} {n}")
        people_line = ' / '.join(note_parts)
        case_lines = []
        for cd in m['case_descs']:
            case_lines.append(f"案件: {cd}")
        for cn in m['case_nos']:
            case_lines.append(f"案号: {cn}")
        court_name_merged = '、'.join(sorted(m['courts'])) if m['courts'] else court_name

        # 拼接完整备注：多人合一 + 案件信息
        note_parts_full = []
        if len(m['names']) > 1:
            note_parts_full.append(' / '.join(f"{r} {n}" for (n, r) in m['names']))
        else:
            note_parts_full.append(f"{m['best_role']} {m['best_name']}")
        for cd in m['case_descs']:
            note_parts_full.append(f"案件: {cd}")
        for cn in m['case_nos']:
            note_parts_full.append(f"案号: {cn}")
        note_parts_full.append(f"法院: {court_name_merged}")
        note_parts_full.append('[诉讼中枢]')

        merged_contacts.append({
            'name': display_name,
            'phone': phone_clean,
            'role': m['best_role'],
            'court': court_name_merged,
            'case_no': '; '.join(m['case_nos']),
            'case_desc': m['case_descs'][0] if m['case_descs'] else '',
            'note_text': '\\n'.join(note_parts_full),
            '_all_people': note_parts,
            '_all_cases': case_lines,
        })

    all_contacts = merged_contacts

    # 写入临时 JSON 文件，通过 court_contacts.py 处理
    import tempfile
    tmp = tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False, encoding='utf-8')
    json.dump(all_contacts, tmp, ensure_ascii=False)
    tmp.close()

    contacts_script = os.path.join(SCRIPT_DIR, 'court_contacts.py')
    try:
        r = subprocess.run(
            [sys.executable, contacts_script, 'add-file', tmp.name],
            capture_output=True, text=True, timeout=60
        )
        # add-file 输出多行缩进 JSON，取 stdout 完整 JSON 部分
        try:
            result = json.loads(r.stdout.strip().split('\n', 1)[-1] if '\n' in r.stdout.strip() else r.stdout.strip())
        except json.JSONDecodeError:
            # 多行缩进 JSON 的兜底解析
            json_start = r.stdout.find('{')
            json_end = r.stdout.rfind('}') + 1
            if json_start >= 0 and json_end > json_start:
                result = json.loads(r.stdout[json_start:json_end])
            else:
                result = {}
        
        # 同时导出 vCard 到桌面（手机可直接导入）
        contacts_json = json.dumps(all_contacts, ensure_ascii=False)
        vcf_r = subprocess.run(
            [sys.executable, contacts_script, 'export', contacts_json],
            capture_output=True, text=True, timeout=15
        )
        vcf_path = vcf_r.stdout.strip().split('\n')[-1] if vcf_r.stdout.strip() else ''

        os.unlink(tmp.name)
        return {
            'saved': len(result.get('created', [])) + len(result.get('updated', [])),
            'created': len(result.get('created', [])),
            'updated': len(result.get('updated', [])),
            'failed': len(result.get('failed', [])),
            'vcf_path': vcf_path if os.path.isfile(vcf_path) else '',
...
def create_all_reminders(report_json_or_file):
    ...
    for doc in docs:
        hearing_time = doc.get('hearing_time') or ''
        hearing_location = doc.get('hearing_location') or ''
        case_type = doc.get('case_type') or '案件'
        ...
        if doc_type in ('判决书', '裁定书'):
            ...
            _args = [sys.executable or 'python3', deadline_script, 'setup',
                     case_no, case_type, doc_type, '', sent_at, court]
            print(f"[DEBUG] args types: {[type(a).__name__ for a in _args]}", file=sys.stderr)
            print(f"[DEBUG] args values: {_args}", file=sys.stderr)
            try:
                r = subprocess.run(_args, capture_output=True, text=True, timeout=30)
            except Exception as e:
                print(f"[DEBUG] subprocess crashed: {type(e).__name__}: {e}", file=sys.stderr)
                import traceback
                traceback.print_exc()
                r = None
            print(r.stdout, file=sys.stderr)
...
示例:
  ...
  %(prog)s --photo /path/to/传票.jpg
    ...
    # 第四步：法院人员通讯录保存（传票/受理通知书自动触发）
    contacts_result = {}
    court_contacts_docs = [d for d in docs if d['document_type'] in ('传票', '受理案件通知书', '应诉通知书', '裁定书', '举证通知书')]
    if court_contacts_docs:
        case_no = sms_info.get('case_no', docs[0].get('case_no', '')) if docs else ''
        court_name = download_result.get('court_name', '')

        # 构建人话版案件描述："张某诉李某等 民间借贷纠纷"
        case_type = ''
        for d in docs:
            if d.get('case_type'):
                case_type = d['case_type']
        ...
        defendant = next((p['name'] for p in parties if p['role'] == 'defendant'), None)
        if plaintiff and defendant:
            case_desc = f"{plaintiff}诉{defendant} {case_type}" if case_type else f"{plaintiff}诉{defendant}"
        elif plaintiff:
            case_desc = f"{plaintiff} {case_type}" if case_type else plaintiff
        else:
            case_desc = case_type or ''

        print(f"\n👤 第四步：保存法院人员通讯录...", file=sys.stderr)
        contacts_result = save_court_contacts(court_contacts_docs, court_name, case_no, case_desc)
        if contacts_result.get('saved', 0) > 0:
            print(f"   ✅ 已保存 {contacts_result['saved']} 位联系人到通讯录", file=sys.stderr)
            for c in contacts_result.get('contacts', []):
                print(f"      {c.get('role', '')}: {c.get('name', '')} - {c.get('phone', '')}", file=sys.stderr)
            if contacts_result.get('vcf_path'):
                print(f"   📱 vCard 已导出: {contacts_result['vcf_path']}（可发手机导入）", file=sys.stderr)
        else:
            print(f"   ℹ️  {contacts_result.get('message', '未提取到联系人信息')}", file=sys.stderr)

    # 第五步：归档
    archive_result = {}
    if not args.no_archive and download_result.get('files'):
        print(f"\n📁 第五步：归档...", file=sys.stderr)
    ...
    report['contacts'] = contacts_result
    ...
    if not args.json_only:
        ...
        # 通讯录保存结果
        if contacts_result.get('saved', 0) > 0:
            print(f"   👤 通讯录: {contacts_result['saved']} 位法院人员已保存", file=sys.stderr)
            if contacts_result.get('vcf_path'):
                print(f"   📱 vCard: {contacts_result['vcf_path']}", file=sys.stderr)
