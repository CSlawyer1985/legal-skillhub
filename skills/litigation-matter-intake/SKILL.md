...
1. 加载 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` → 风险校准（用于分流）、执业背景（用于上下文、冲突检索方法）、相关方（用于抄送人员）。
2. 按以下工作流操作。
3. 运行统一登记：标识信息、利益冲突检索、来源、风险分流、重要性、外聘律师、内部负责人、证据保全、关键日期、初始姿态。
4. 从案件名称生成代号（小写、连字符、年份）。
5. 创建 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/matter.md` —— 完整记述式登记。
6. 创建 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/history.md` —— 以本次登记作为首条记录。
7. 追加结构化行至 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/_log.yaml`。
...
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` —— 风险校准（分流门槛、重要性、和解阶梯）、执业背景（相关方、外聘律师库）。
- `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/_log.yaml` —— 确认代号唯一性。
...
- 案由（依据最高人民法院《民事案件案由规定》）`[元典法规]`
- 我方地位：`原告 | 被告 | 申请人 | 被申请人 | 被调查人`
  - 如业务规范的默认角色已设定，据此预填充并确认。如默认角色为"因案而异"，直接询问。
- 管辖（受理法院、仲裁机构或行政机关）

### 2. 利益冲突检索

在进一步操作前，按 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` → 利益冲突审查运行冲突检索步骤。

- **状态：** `已通过 | 待定 | 未运行 | 已豁免`
- **方式：** 匹配 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 声明的方式。
- **审查人：** 姓名/团队/律所
- **审查日期：** YYYY-MM-DD
...
- `未运行` → **停止。此处为门禁。** 冲突姿态解决前不创建 `matter.md`、`history.md` 或 `_log.yaml` 条目。三条可接受路径：
  ...
  **路径1 —— 现在运行冲突检索。** 暂停本登记。按 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 的冲突审查完成。返回时带 `status: cleared` 或 `status: waived` 附理由。

  **路径2 —— 标注待定，附负责人+截止日期。** 仅在 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 的冲突审查声明允许平行登记时可用。记录：谁在运行冲突检索、预计何时返回、检查哪些主体。登记继续；案件行携带 `conflicts.status: pending`；`/portfolio-status` 每次运行均标注；`/matter-update` 重复提示直至解决。
...
- 严重性：高 | 中 | 低（参考 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 的严重性分级）
- 可能性：高 | 中 | 低（参考可能性分级）
...
对照 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 中的事务所门槛：
- `需计提 | 已披露 | 监控中 | 不适用`
...
来自 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/profile.md` 执业背景——哪些内部相关方需要参与？
- 业务负责人
- HR 负责人（如是劳动争议）
- 公关联系人（如有声誉风险）
- 信息安全负责人（如涉及数据或网络安全）
...
### `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/matter.md`
...
### `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/[slug]/history.md`
...
### 追加至 `$LEGAL_AGENT_PROFILE_HOME/litigation-legal/matters/_log.yaml`
...
- **自行运行利益冲突检索。** 记录结果、状态、方式和审查实体。实际审查在事务所业务规范声明的不论什么系统（或判断）中完成。如用户说"已通过"，本技能按面值接受并记录元数据。
