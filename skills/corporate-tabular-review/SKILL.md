...
1. 加载 `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` → 尽调结构、阈值、内部格式。
2. 确认：什么文件、什么列、输出到哪里。
...
corporate-tabular-review --schema .review-schema.yaml --docs ./vdr/02-Contracts/
corporate-tabular-review --template ma-diligence
```
...
- `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/profile.md` → 尽调结构、重要性阈值、内部格式偏好
- `$LEGAL_AGENT_PROFILE_HOME/corporate-legal/deals/[代码]/deal-context.md`（如处理特定交易）
...
**Excel**（`.xlsx`）或**在线表格**——取决于用户的工作环境。询问；不猜测。两者遵循相同的工作簿结构（见 `excel-output.md` 和 `gsheets-output.md`）。对 Excel：如可用则用 Excel 代理（Office 代理），`openpyxl` 为备选。对 Sheets：如可用则用 Sheets MCP，通过 ADC 使用 Sheets API，CSV 导入为备选。在电子表格输出中：
