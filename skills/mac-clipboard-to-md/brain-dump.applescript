on run
    try
        ...
        -- 设置文件夹路径（请修改为你自己的路径）
        set folderPath to "~/你的Obsidian库/brain dump/"
        ...
        set yamlFrontmatter to "---" & linefeed

        if userTags is not "" then
            set tagList to my splitString(userTags, ",")
            set yamlFrontmatter to yamlFrontmatter & "tags:" & linefeed
            repeat with aTag in tagList
                set cleanTag to my trim(aTag)
                if cleanTag is not "" then
                    set yamlFrontmatter to yamlFrontmatter & "  - " & cleanTag & linefeed
                end if
            end repeat
        end if
        ...
        if button returned of descDialog is "确认" and userDesc is not "" then
            set yamlFrontmatter to yamlFrontmatter & "description: " & userDesc & linefeed
        end if

        set yamlFrontmatter to yamlFrontmatter & "---" & linefeed & linefeed

        -- 构建完整的文件内容
        set fileContent to yamlFrontmatter
        set fileContent to fileContent & "# "& dateString & " " & timeString & linefeed & linefeed
        set fileContent to fileContent & "**创建时间**: " & (time string of currentDate) & linefeed & linefeed
        set fileContent to fileContent & "## 内容" & linefeed & theClipboard & linefeed & linefeed
        set fileContent to fileContent & "---" & linefeed & "*自动生成*" & linefeed

        -- 写入文件
        set escapedContent to quoted form of fileContent
        set escapedPath to quoted form of filePath
        do shell script "export LANG=en_US.UTF-8; printf '%s' " & escapedContent & " > " & escapedPath
