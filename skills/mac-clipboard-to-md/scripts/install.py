...
def interactive_install():
    ...
    for name in tools_to_install:
        if name == "work-record":
            opts = {"桌面": "~/Desktop/Work record.md", "用户目录": "~/Work record.md"}
        else:
            opts = {"桌面": "~/Desktop/brain dump/", "用户目录": "~/brain dump/"}
        print(f"\n{name} 保存路径：")
        for k, v in opts.items():
            print(f"  [{k}] {v}")
        choice = input("选一个，或直接输自定义路径（回车默认桌面）: ").strip()
        if not choice or choice == "桌面":
            user_path = opts["桌面"]
        elif choice == "用户目录":
            user_path = opts["用户目录"]
        else:
            user_path = choice
        install_plan[name] = os.path.expanduser(user_path)
