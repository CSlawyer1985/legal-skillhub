...
```json
{
  ...
  "options": [
    {
      "label": "~/Desktop/Work record.md",
      "description": "桌面，一定存在"
    },
    {
      "label": "~/Work record.md",
      "description": "用户目录根"
    },
    {
      "label": "自定义路径",
      "description": "自己输入完整路径"
    }
  ]
}
```

- 选 `桌面` → `path = expanduser("~/Desktop/Work record.md")`
- 选 `用户目录` → `path = expanduser("~/Work record.md")`
- 选 `自定义路径` → 弹出第二层选项：选「Vault 根目录」自动检测 vault 路径，或选「Other」自行输入（含子目录）
...
- 选 `桌面` → `path = expanduser("~/Desktop/brain dump/")`
- 选 `用户目录` → `path = expanduser("~/brain dump/")`
- 选 `自定义路径` → 弹出第二层选项：选「Vault 根目录」自动检测 vault 路径，或选「Other」自行输入（含子目录）
...
WORK_PATH="~/Desktop/Work record.md"
# 如果是 brain-dump，路径示例：
BRAIN_PATH="~/Desktop/brain dump/"
...
读取模板，搜索路径占位符定位需要修改的行：

```bash
# 搜索占位路径，不依赖行号
grep -n '你的Obsidian库' "<skill_dir>/work-record.applescript" "<skill_dir>/brain-dump.applescript"
```

然后直接告诉用户改哪里（将「你选的路径」替换为 Phase 4 中用户实际选择的路径）：

```
找到包含「你的Obsidian库」的行，把路径改成你自己的。

需要改的文件在：<skill_dir>/

--- work-record ---
  原路径：set filePath to "~/你的Obsidian库/Work record.md"
  改成：  set filePath to "你选的路径"

--- brain-dump ---
  原路径：set folderPath to "~/你的Obsidian库/brain dump/"
  改成：  set folderPath to "你选的文件夹路径"

改好之后分别全选复制。

接下来创建 macOS 快速操作（每个工具都要创建一个）：
