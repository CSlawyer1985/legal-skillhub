...
# 这三个是不同的量，先前混用过，必须分清（Astra 第四轮 2.3）：
#   STUB（在 relation_core）　端部必须留的直段，决定 25.12 / 37.12 的接入预算
#   HUG_NEAR　　　　　　　　　 平行于主体边走算不算贴边的判据阈值
#   MIN_CLEAR　　　　　　　　　通道候选离主体多近就不生成
# 把 STUB 从 22 改到 36 曾让某案不再贴边，但那只说明路线变了，
# 不等于 STUB 是贴边参数。过滤过近的通道候选也不能替代端部预算——
# 端口就在主体边上，把近端点一律删掉会破坏合法接入。
