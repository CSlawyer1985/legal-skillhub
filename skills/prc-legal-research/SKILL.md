...
1. 优先使用元力平台已绑定的 `yuandian-law` 和 `yuandian-case` MCP。
2. 分别执行 `tools/list`，确认所需工具存在，并以运行时 `inputSchema` 为参数唯一真相源。
3. 平台 MCP 已绑定时，不读取、回显或写入 API Key，不修改全局 MCP 配置。
4. 平台没有 MCP 时，才使用包内脚本；凭据优先从环境变量 `YUANDIAN_API_KEY` 读取，旧版同目录 `config.py` 仅作兼容回退。
5. Tavily 只用于二手文献；未配置时跳过该阶段并明示说明，不影响元典一手资料检索。

当 MCP 返回 401/403、429、超时或缺少能力时，应分别报告 `AUTH_FAILED`、`RATE_LIMITED`、`NETWORK_FAILED` 或 `CAPABILITY_MISSING`，不得使用模型记忆补造一手资料。
...
| Server | Streamable HTTP 入口 | 现行能力 |
|---|---|---|
| `yuandian-law` | `https://open.chineselaw.com/mcp/law/stream` | 法规/法条语义检索、关键词检索和详情 |
| `yuandian-case` | `https://open.chineselaw.com/mcp/case/stream` | 案例语义检索、权威/普通案例检索和详情 |

平台已绑定 MCP 时，直接调用平台发现到的工具。本地 CLI 可用包内兼容层：

```bash
PYTHONIOENCODING=utf-8 python -c "
import sys, json
sys.path.insert(0, '$SCRIPTS')
from yuandian_mcp import list_tools, call_tool
tools = list_tools('law')
tool = next(t for t in tools if t['name'] == 'yuandian_rh_ft_search')
print(json.dumps(tool['inputSchema'], ensure_ascii=False, indent=2))
result = call_tool('law', tool['name'], {'keyword': '{关键词}'})
print(json.dumps(result, ensure_ascii=False, indent=2))
"
```

调用纪律：
- MCP 传输为 Stateless Streamable HTTP `POST`，认证为 `Authorization: Bearer <YUANDIAN_API_KEY>`。
- `Accept` 同时包含 `application/json` 和 `text/event-stream`，必须兼容 JSON 与 SSE 响应。
- 不根据文档静态拼接工具名或参数；每次以 `tools/list` 和运行时 `inputSchema` 为准。

### 一手权威资料 — 元典开放 API（兼容回退）
...
可用函数：
- `search_law_vector(query, rewrite_flag, fatiao_filter, return_num)` — 法律法规语义检索
...
- `search_case_vector(query, rewrite_flag, wenshu_filter, return_num)` — 案例语义检索
...
3. **来源透明**：
   - 所有法条必须经元典 MCP 或开放 API 验证后引用，无需在正文中标注来源标签
...
> **硬性暂停**：未收到用户确认，禁止开始任何检索工作（包括 Tavily 和元典 MCP/API 调用）。
...
从检索结果提取：
1. **引用的法律法规清单** — 法规名称 + 法条号（待元典 MCP/API 验证）
2. **引用的案例** — 案号或案件名（待元典 MCP/API 验证）
3. **扩展关键词** — 用于第五阶段检索
...
对每条提取的法规/法条优先调用元典 MCP 核实；平台未绑定 MCP 时才用开放 API 兼容回退：
```python
...
向用户汇报本次工具使用情况：
- **元典 MCP/API**：使用了哪些工具或函数、调用几次、分别检索了什么
...
- 禁止编造法条。所有法条必须通过元典 MCP 或开放 API 获取原文，返回为空时如实告知，不用 AI 记忆替代。
- 禁止编造案例。所有案例必须来自元典 MCP 或开放 API 检索结果。
