#!/usr/bin/env python3
"""
generate_contract.py — 合同 Word 文档生成工具
...
import os
    ...
    sections = [
        ...
        {"type": "chapter", "text": "第一章  定义与解释"},
        {"type": "article", "text": "第1.1条  定义"},
        {"type": "body", "text": "正文内容..."},
        {"type": "signature", "party_a": "甲方名称", "party_b": "乙方名称"},
    ]

    generate_contract(sections, output_path)

支持的 section type
- title: 居中大标题 (22pt 黑体加粗)
- subtitle: 居中副标题 (16pt 黑体加粗)
- chapter: 居中章标题 (14pt 黑体加粗)
- article: 条标题 (12pt 黑体加粗，首行缩进2字符)
...
def generate_contract(sections, output_path=None):
    """
    Generate a Word document from structured section data.

    Args:
        sections: List of dicts, each with 'type' and content keys
        output_path: Path to save the .docx file (optional; auto-generated if None)
    ...
    if output_path is None:
        output_path = build_filename('contract', 'party', 'matter')
    out_dir = os.path.dirname(output_path)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    doc.save(output_path)
    print(f"合同文档已生成: {output_path}")
    return output_path
...
if __name__ == '__main__':
    # Demo
    sections = [
        {"type": "title", "text": "服务合同"},
        {"type": "right", "text": "合同编号【___________】"},
        {"type": "chapter", "text": "第一章  定义与解释"},
        {"type": "article", "text": "第1.1条  定义"},
        {"type": "body", "text": "除本合同另有约定外，下列术语具有如下含义"},
        {"type": "body", "text": "（1）服务指乙方根据本合同约定向甲方提供的全部服务。"},
        {"type": "signature", "party_a": "【甲方公司名称】", "party_b": "【乙方公司名称】"},
    ]
    generate_contract(sections, '/sandbox/workspace/outputs/demo_contract.docx')
