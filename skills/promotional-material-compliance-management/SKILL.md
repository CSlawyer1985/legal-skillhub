...
授权脚本会优先读取包内 `_meta.json` 的 `slug` 作为服务端 `skillCode`。`slug` 必须是平台维护的技能编号，例如 `BT_XCCLHGGL`，不是 Skill 展示名称。只有包内缺少 `_meta.json` 时，才需要手动传 `--skill-code`。
...
触发：`~/.workbuddy/skill_auth/BT_XCCLHGGL.json` 不存在。
...
（本地未找到本 Skill 的凭证缓存：~/.workbuddy/skill_auth/BT_XCCLHGGL.json）
...
2. 或联系管理员核实 skillCode "BT_XCCLHGGL" 是否对你的账号开放
...
3. 若你认为不应出现此提示，请联系管理员核实 skillCode "BT_XCCLHGGL" 的权限配置
