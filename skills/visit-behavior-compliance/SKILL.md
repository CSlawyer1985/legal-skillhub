...
授权脚本会优先读取包内 `_meta.json` 的 `slug` 作为服务端 `skillCode`。`slug` 必须是平台维护的技能编号，例如 `BT_BFXWHG_20260621`，不是 Skill 展示名称。只有包内缺少 `_meta.json` 时，才需要手动传 `--skill-code`。
...
3. 如仍失败，联系管理员核实 skillCode "BT_BFXWHG_20260621" 的权限配置
...
2. 或联系管理员核实 skillCode "BT_BFXWHG_20260621" 是否对你的账号开放
...
3. 若你认为不应出现此提示，请联系管理员核实 skillCode "BT_BFXWHG_20260621" 的权限配置
