...
| 审查规则文件 | `<skill_root>/Checking Rules/32-权利要求书-复杂审查1.md` |
| 所需章节文件 | `<work_dir>/section_claims_<timestamp>.json` |
| 禁止读取 | 其他任何章节拆分文件 |
| 禁止审查章节 | 摘要、说明书 |
| 输出文件 | `<work_dir>/reviews_agent7_<timestamp>.json` |
...
1. 读取审查规则文件 `<skill_root>/Checking Rules/32-权利要求书-复杂审查1.md` 的完整内容
2. 读取待审查文本：读取 `<work_dir>/section_claims_<timestamp>.json`，提取 `paragraphs` 字段的值作为待审查文本
3. 执行审查：按照审查规则，需要区分独立权利要求和从属权利要求；关注保护范围不清楚、权利要求不简要
4. 输出审查意见JSON：保存为 `<work_dir>/reviews_agent7_<timestamp>.json`，section 填写 "权利要求书"
