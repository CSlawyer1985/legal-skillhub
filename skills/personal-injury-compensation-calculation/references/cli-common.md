...
适用于 `personal-injury-compensation-calculation@1.0.0`。
...
```bash
npx @delilegal/deli-cli@latest cmds personal-injury-compensation-calculation@1.0.0
```
...
- 只调用人身损害赔偿计算、参数校验、统计参数获取、结果生成等计算相关命令。
- 不调用法规检索命令。
- 不调用案例检索命令。
- 稳定公式、赔偿项目、统计数据口径和法条索引以本 skill 的 references 为准。
