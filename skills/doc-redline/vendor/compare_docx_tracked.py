...
def main():
    ...
    # 【本地补丁 1 · 2026-09-16】上游原为 r'\s+|\w+|[^\w\s]+'。
    # 末尾的 + 会把「连续标点」粘成一个不可分的 token。中文文书里
    # 「），」与「）。」这类"前一字相同、后一字不同"的相邻标点变化极常见，
    # 粘成整块后两块互不相等 → 被判成"删一块 + 插一块"，
    # 于是**两版里都在同一位置的括号、句号被标成删除+插入**，
    # 看起来像"凭空多了一个括号"，会被对方拿来说事。
    # 改为单字符标点后，「）」能被正常匹配保留，标记只落在真正变化的那一个标点上。
    # 回归自证：scripts/regress_trackdiff.py（9 个场景，只改善不劣化）。
    # 回退：本目录 compare_docx_tracked_upstream.py（上游原版）。
